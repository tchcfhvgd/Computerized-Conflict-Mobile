...
import sys.io.File;
#end
import OptionsState.PreferencesSubstate;
...
class TitleScreenNew extends MusicBeatState
{
	...
	public static var instance:TitleScreenNew;
	...
	override public function create():Void
	{
		...
		if(save.data.comeToOld == null) {
			save.data.comeToOld = huhthing;
		}
		huhthing = save.data.comeToOld;
		save.flush();
		...
		if(!initialized && huhthing)
		{
			FlxTransitionableState.skipNextTransIn = true;
			FlxTransitionableState.skipNextTransOut = true;
			MusicBeatState.switchState(new TitleState());
			Lib.application.window.title = "Animation VS. Friday Night Funkin': Deluxe Edition";
		}
	}
	...
	function startIntro()
	{
		if (!initialized)
		{
			if(FlxG.sound.music == null) {
				FlxG.sound.playMusic(Paths.music('freakyMenuNew'), 0);
			}
		}
